# comb and the other Rust Hive libraries

`comb` is not the first Rust library for Hive. This document says what the others do,
what `comb` took from looking at them, and — since the question deserves a straight
...
| `comb` | 0.1.0 | unpublished | This crate |

`hive-xylem` is the closest comparison: a general-purpose SDK with overlapping goals.

---

## Is xylem more mature than comb?

**On the one measure that matters most — production exposure — neither is mature, and
xylem is slightly ahead of comb.** It is published on crates.io with five releases;
`comb` is not published and has never had a transaction accepted by a Hive node. 99
downloads is not adoption, but it is more than zero.

On every other measure, `comb` is substantially larger and more verified. Both facts
...
advantage `comb` does not have, its code is clean, and it avoids `unsafe` entirely. If
...
`comb` covers far more of the protocol, is verified against a reference implementation
...
## What comb changed after reading xylem
...
Given a set of public keys, does it satisfy an authority? `comb` had
...
authority satisfied through a delegated account reports `false`. `comb`'s version
...
Exposed as `comb.check_authority()` in Python, `Account.verify_account_authority()` in
...
`comb` had streaming in Python but not in Rust. `NodeClient::stream_blocks` is a lazy
...
Small conveniences xylem had that `comb` only had on the Python side.
...
catch, and `comb` found two of its own the same way.
...
its encrypted memos cannot be read by Keychain, hive-js, dhive, beem or `comb`, and it
...
## What comb deliberately does not do

**Async.** `comb` is synchronous, with a `Transport` trait as the only network seam.
...
vary by deployed app. `comb`'s `NodeClient::call` reaches anything hived exposes over
JSON-RPC, but HAF is a different protocol and a moving target. xylem ships a minimal
client (reputation lookups); `comb` ships none rather than a half-one.
