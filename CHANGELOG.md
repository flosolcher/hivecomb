...
point: the tarball held `LICENSE`, `README.md` and a `package.json` declaring
`main: index.js`, and no `index.js`. `npm i hivecomb` installed and then threw
`MODULE_NOT_FOUND`. npm does not allow a version to be republished, so the fix is this
release.

`index.js` and `index.d.ts` are generated by `napi build` and had been gitignored. The
release job never builds — it downloads prebuilt `.node` artifacts — so they were absent
from that checkout and `npm publish` packed what it found. They are committed now, which
is the napi-rs convention and exactly the reason for it, and CI rebuilds them and fails
if the tree disagrees.

Both rehearsals ran `npm publish --dry-run` and passed, because publishing an incomplete
